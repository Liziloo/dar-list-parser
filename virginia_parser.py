...
SERVICE_PATTERN = re.compile(
    "(" + "|".join(re.escape(s) for s in SERVICE_WORDS) + r")",
    re.IGNORECASE,
)
...
def extract_race(text: str):
    """
    Extract race-related phrases, return (race_string, remaining_text, race_detail_if_collapsed).
    If race is African-descended, we normalize to 'AA' and return details separately.
    """
    races = RACE_PATTERN.findall(text)
    if races:
        text = RACE_PATTERN.sub("", text)
        race_detail = "; ".join(sorted(set(r.strip() for r in races)))
        if any(ind in race_detail.lower() for ind in AFRICAN_INDICATORS):
            # Normalize to AA, but preserve detail separately
            return "AA", text.strip(), race_detail
        return race_detail, text.strip(), ""
    return "", text, ""
...
def extract_service(text: str):
    """
    Extract service-related terms from text.
    Returns (service_string, remaining_text).
    """
    services = SERVICE_PATTERN.findall(text)
    if services:
        text = SERVICE_PATTERN.sub("", text)
        service_detail = "; ".join(sorted(set(s.strip().title() for s in services)))
        return service_detail, text.strip()
    return "", text
...
def parse_record(line: str):
    """
    Given a fused record line, return the 9 output columns:
    ...
    if m:
        surname = m.group(0).strip()  # literal phrase
        ...
        # Service
        service, remainder = extract_service(remainder)
        ...
        return ["", "", surname, "", race, owner, service, "; ".join(sources), notes]
    ...
    # 3. Race extraction
    # --------------------------------------------------------
    race, rest, race_detail = extract_race(rest)
    ...
    # --------------------------------------------------------
    service, rest = extract_service(rest)
    ...
    # If race_detail exists (we collapsed to AA), store in notes as Race detail
    if race == "AA" and race_detail:
        if notes:
            notes = f"{notes}; Race detail: {race_detail}"
        else:
            notes = f"Race detail: {race_detail}"

    return ["", "", surname, first_name, race, owner, service, "; ".join(sources), notes]
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse Revolutionary War lists into pipe- or comma-delimited CSV."
    )
