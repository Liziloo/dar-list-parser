...
def is_african_descent_text(text: str) -> bool:
    """
    Decide whether a race-related chunk indicates African descent,
    in which case we normalize Race to 'AA'.
    """
    t = text.lower()
    if any(ind in t for ind in AFRICAN_INDICATORS):
        return True
    if any(root in t for root in RACIAL_NAME_ROOTS):
        return True
    if any(note in t for note in RACE_NOTE_KEYWORDS):
        return True
    return False
...
def extract_paren_notes(line: str):
    ...
    # Comma-normalization
    cleaned = re.sub(r"\s*,\s*,\s*", ",", cleaned)   # collapse ",  ,"
    cleaned = re.sub(r",+", ",", cleaned)            # collapse " ,,," → ","
...
def extract_owner_from_chunks(chunks):
    """
    Look for enslaver/owner phrases in the chunks.
    Returns (cleaned_chunks, owner_string).
    """
    owners = []
    new_chunks = []
    ...
    for chunk in chunks:
        if not chunk:
            continue
        found = OWNER_PAT.findall(chunk)
        if found:
            owners.extend(o.strip() for o in found)
            chunk = OWNER_PAT.sub("", chunk).strip(" ;,")
        if chunk:
            new_chunks.append(chunk)

    owner_str = "; ".join(owners) if owners else ""
    return new_chunks, owner_str


def is_service_chunk(chunk: str) -> bool:
    if not chunk:
        return False
    t = chunk.strip().lower()
    if t in SERVICE_KEYWORDS:
        return True
    # Handle things like "Private," / "Soldier."
...
def parse_line(line: str, log_list, lineno, filename):
    ...
    # Strip aliases out & capture them
    tail_for_rest, alias_names = strip_aliases_from_chunks(tail_for_rest)

    # Extract owner phrases from the remaining chunks
    tail_for_rest, owner_field = extract_owner_from_chunks(tail_for_rest)
    ...
    for chunk in tail_for_rest:
        ...
        elif is_service_chunk(chunk):
            service_chunks.append(chunk)
        else:
            source_pieces.append(chunk)

    # Build notes (source notes + location, race details if we normalize)
    source_notes_parts = list(source_note_parens)
    if location_chunk:
        source_notes_parts.append(location_chunk)

    # Race field: normalize African-descent to "AA" and stash full phrases in notes
    race_field = ""
    if race_chunks:
        if any(is_african_descent_text(rc) for rc in race_chunks):
            race_field = "AA"
            race_detail = "; ".join(race_chunks)
            source_notes_parts.append(f"Race detail: {race_detail}")
        else:
            race_field = ", ".join(race_chunks)

    service_field = "; ".join(service_chunks) if service_chunks else ""
    ...
    # Reattach alias information to given name if present
    if alias_names:
        if given:
    ...
    # Unified 9-column schema:
    # ["", "", SURNAME, FIRST, RACE, OWNER, SERVICE, SOURCES, NOTES]
    return ["", "", surname, given, race_field, owner_field, service_field, sources_field, source_notes_field]
...
def main():
    parser = argparse.ArgumentParser(description="Parse Revolutionary War rolls into structured CSV/PSV rows.")
